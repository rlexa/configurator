...
namespace Configurator
{
    ...
    public class Context : IContext
    {
        ...
        {
            System.Console.Out.WriteLine(DateTime.UtcNow.ToString("hh:mm:ss") + " Configurator" + (context == null ? "" : " " + context.GetType().Name) + ": " + log);
        }
        ...
        {
            Context.addReplaceBeanProperty(bean, property);
        }

        /// <summary>Adds or replaces the factory property.</summary><param name="bean"></param><param name="property"></param>
        public void addBeanFactory(BeanData bean, BeanData.BeanProperty property)
        {
            Context.addReplaceBeanFactory(bean, property);
        }
        ...
        private object inflate(BeanData bean)
        {
            ...
            try
            {
                ...
                else if (!clazz.IsAssignableFrom(instance.GetType()))
        ...
        private object createInstance(string infoTag, BeanData bean, Type clazz)
        {
            ...
            else
            {
                ...
                {
                    Context.findAppropriateMethod(false, null, clazz.GetConstructors(), vals, out methodFirstPerfect, out valsPerfect, out methodFirstOk, out valsOk);
                    MethodBase methodUse = methodFirstPerfect != null ? methodFirstPerfect : methodFirstOk;
                    if (methodUse != null)
                    {
                        try { ret = (methodUse as ConstructorInfo).Invoke(methodUse == methodFirstPerfect ? valsPerfect : valsOk); }
                        catch (Exception) { }
                    }
                }
                else
                {
                    Context.findAppropriateMethod(true, bean.factory.name, clazz.GetMethods(), vals, out methodFirstPerfect, out valsPerfect, out methodFirstOk, out valsOk);
                    MethodBase methodUse = methodFirstPerfect != null ? methodFirstPerfect : methodFirstOk;
                    if (methodUse != null)
                    {
                        try { ret = (methodUse as MethodInfo).Invoke(null, methodUse == methodFirstPerfect ? valsPerfect : valsOk); }
                        catch (Exception) { }
                    }
                }
            }
            ...
            for (int ii = 0; (methodFirstOk == null || methodFirstPerfect == null) && ii < methods.Length; ++ii)
            {
                ...
                if (method.GetParameters().Length == (vals == null ? 0 : vals.Length) && method.IsStatic == bStaticMethods && (string.IsNullOrEmpty(optNameMethod) || method.Name == optNameMethod))
                {
                    ...
                    for (int jj = 0; jj < vals.Length; ++jj)
                    {
                        ...
                        if (value == null || typeParam.IsAssignableFrom(value.GetType()))
                        {
                ...
                case BeanData.BeanProperty.BeanValueCollection.BeanValueCollectionType.BVCT_MAP:
                    {
                        ...
                        MethodInfo method = clazzConstructed.GetMethod("Add", clazzParams);
                        if (method == null)
                ...
                case BeanData.BeanProperty.BeanValueCollection.BeanValueCollectionType.BVCT_SET:
                    {
                        ...
                        {
                            MethodInfo methodToArray = clazzConstructed.GetMethod("ToArray");
